...
//서버 Post
Future<String> call(String no,String loc_code, String token_key,String author, String regdate, String rating,
    String content, List<File> pickedimage) async {
  Map<String, String> headers = { "Accesstoken": "access_token"};

  var url = Uri.parse('http://dongseok1.dothome.co.kr/treveat/mReview/modifyPage.php');
  ...
  if(pickedimage.length==1){
    image_byte = pickedimage[0].readAsBytesSync();
    request.fields['imglink']=base64Encode(image_byte);
  }
  else if(pickedimage.length==2){
    image_byte = pickedimage[0].readAsBytesSync();
    request.fields['imglink']=base64Encode(image_byte);

    image_byte = pickedimage[1].readAsBytesSync();
    request.fields['imglink2']=base64Encode(image_byte);
  }
...
class review_modify extends StatefulWidget {//no도 같이 받아와서 아래 바로 대입
  String loc_code_="";
  String no="";
  review_modify(this.loc_code_,this.no,{Key? key}) : super(key:key);

  @override
  State<review_modify> createState() => _review_modify();
}

class _review_modify extends State<review_modify> {
            ...
            child:(isLoading==false)?ElevatedButton(
              onPressed: () async {
                if (formkey.currentState!.validate()) {
                  ...
                  await call(no,loc_code,_token_key,author,regdate,rating,content,pickedimage).then((value) => setState(() {
                    response = value;
                  }));
                  setState(()  {
                    isLoading=false;

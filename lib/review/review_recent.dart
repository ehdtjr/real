...
Future<List<model_review>> fetchReview(String loc_code, List<String> image) async {
  final response = await http.get(Uri.parse(
      'http://dongseok1.dothome.co.kr/treveat/mReview/selectValue.php?start=0&end=20&dsort=desc&loc_code=$loc_code'));
...
class riview_recent extends StatefulWidget {
  String loc_code_;
  riview_recent(this.loc_code_,{Key? key}) : super(key:key);

  @override
  State<riview_recent> createState() => _riview_recent();
}

class _riview_recent extends State<riview_recent> {
